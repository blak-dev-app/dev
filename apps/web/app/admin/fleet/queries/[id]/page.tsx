...
export default function FleetQueryDetailPage() {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
          <div className="rounded-xl border border-border bg-card p-6 xl:col-span-2">
            ...
            {data.driverName ? (
              <div className="mt-1 text-xs text-muted-foreground">Added by: {data.driverName}</div>
            ) : null}
            <div className="mt-6">
            ...
            <div className="mt-4">
              ...
              <p className="text-sm text-muted-foreground">{data.message || data.description || "—"}</p>
            </div>
          </div>
              ...
              {messages.length === 0 ? (
                ...
                messages.map(({ id, data: m }) => (
                  <div key={id} className="rounded-lg bg-muted/40 p-2 text-xs">
                    <div className="font-medium">{m.sender === "fleet" ? "Fleet Admin" : m.sender === "admin" ? "Super Admin" : "Driver"}</div>
