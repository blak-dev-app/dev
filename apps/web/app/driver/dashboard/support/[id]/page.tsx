...
export default function DriverSupportDetailPage() {
  ...
  // Same unified `tickets` collection as the Fleet/Super Admin ticket
  // detail pages — see lib/types.ts Ticket / TicketMessage. firestore.rules
  // permits a driver to read/update a ticket where resource.data.driverId
  // == myDriverId(), which this doc satisfies since it was created by
  // AddTicketModal with driverId set to the signed-in driver's own id.
  React.useEffect(() => {
    if (!params?.id) return
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
          <div className="rounded-xl border border-border bg-card p-6 xl:col-span-2">
            ...
            <div className="mt-1 text-xs text-muted-foreground">
              Raised with: {data.audience === "fleet_admin" ? "Fleet Admin" : "BLAK Support"}
            </div>
            <div className="mt-6">
                ...
                messages.map(({ id, data: m }) => (
                  <div key={id} className="rounded-lg bg-muted/40 p-2 text-xs">
                    <div className="font-medium">
                      {m.sender === "driver" ? "You" : m.sender === "fleet" ? "Fleet Admin" : "BLAK Support"}
                    </div>
